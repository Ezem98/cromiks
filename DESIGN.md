...
**Frame / material (estático, aplica aun con arte placeholder):** **passe-partout tipo TCG** — un marco "matte" tier-coded (gradiente diagonal = sheen metálico, `padding` por tamaño en `sizeMap.framePad`, `tierFrame[tier]`) que rodea TODA la foto. El arte va en una **ventana recesada** adentro (inner-shadow = "hundido en el cartón"), no a sangre. Nameplate como panel oscurecido por gradiente sobre el pie de la ventana (SIN border-top: ese hairline se compositaba como una costura oscura de 1px cruzando la carta), pip de rareza (gema rotada; estrella dorada en legendary), número en badge enmarcado. Extras por tier: rare = scanlines sutiles, epic = glow radial violeta, legendary = marco dorado + borde prisma rotando por fuera + glow gold radial.
