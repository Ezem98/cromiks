...
### Bloqueante (P0)
- [x] ~~**🔴 Login de prod manda a `localhost:8080` tras OAuth**~~ — ✅ FIX EN CÓDIGO (rama `fix/auth-callback-redirect-origin`), **pendiente de DEPLOY a prod**. Causa REAL (no era config de Supabase, ni Google/GCP, ni cache del browser): `src/app/auth/callback/route.ts` armaba los redirects con el `origin` de `request.url`, que **detrás del proxy de Railway es el host interno del contenedor (`localhost:8080`)**, no `cromiks.app`. El OAuth completaba bien (la sesión se seteaba; los auth logs de GoTrue mostraban éxito porque el redirect final de la app es POSTERIOR al `/token` y GoTrue no lo logea) pero el redirect final iba al puerto interno. Fix: usar `env.NEXT_PUBLIC_APP_URL` (igual que `/auth/login`). Afecta también el magic-link por email. Ver [[cromiks-auth-url-config]]. **Verificar en prod después de deployar.**
