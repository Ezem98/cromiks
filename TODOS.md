...
- [ ] **Cutover a prod** — `pnpm seed` → `pnpm db:push` → asegurar `pages.is_active=true WHERE id='francia'` (y las demás inactivas) → `pnpm tsx scripts/test-roll-cards.ts` contra prod. Resetear el inventario de las cuentas de prueba.
- [ ] **Dominio + URL compartible** — comprar/apuntar un dominio (el nombre puede quedar "Cromiks" placeholder).
- [ ] **Confirmar flujo end-to-end de usuario NUEVO** en prod con francia-only (signup → onboarding → sobre → abrir → álbum se llena, sin placeholders).
