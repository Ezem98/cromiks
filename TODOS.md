...
### Bloqueante (P0)
- [x] ~~**🔴 Auth de prod rota (Supabase URL config)**~~ — ✅ ARREGLADA + VERIFICADA EN LOGS (2026-06-04). Síntoma: el login de Google en `cromiks.app` redirigía a `http://localhost:8080` (la app mandaba el `redirect_to` correcto pero Supabase caía al Site URL por allowlist/Site-URL mal). El dueño corrigió la URL config en el dashboard, y los **auth logs de GoTrue confirman el flujo Google exitoso end-to-end** (`/authorize`→`/callback` 302→`/token` 200 pkce→evento `Login`, 19:03 UTC; cero 4xx/5xx; `localhost:8080` aparece 0 veces). Si alguien sigue viendo 8080 es **cache del browser** (pestaña previa al fix) → probar en incógnito. Ver [[cromiks-auth-url-config]].
