-- =============================================================================
-- T6: open_pack deja de devolver image_url (pipeline de imágenes)
-- =============================================================================
-- image_url se derivaba de `c.content->'photo'->>'source'`, el campo viejo que
-- T1 migró (ya no existe). La imagen del cromo ahora vive en card_assets y la
-- resuelve el resolver TS (pack-opening/actions.ts vía getCardImageMap). Sacamos
-- la columna del RPC para que pack-opening pase por el MISMO gate de takedown que
-- las otras 4 superficies — un cromo en takedown NO se puede filtrar al abrir un
-- sobre.
--
-- Cuerpo idéntico a 20260529125447_open_pack_explicit_was_replay.sql salvo:
--   1. se quita la columna `image_url text` del RETURNS TABLE
--   2. se quita la expresión CASE de image_url del SELECT del return query
-- =============================================================================
